...
The Spring Boot auto-configuration options are documented in the xref:starters/core.adoc[Core starter] page.
...
You can also use xref:components:others:test-spring-junit6.adoc[camel-test-spring-junit6] to configure tests declaratively. This example uses the `@MockEndpoints` annotation to auto-mock an endpoint:
...
NOTE: This feature requires `management.server.port` to be different from the main application port.
...
With this configuration, Undertow-specific metrics will be automatically exposed under `/actuator/metrics`, providing detailed insights into HTTP server including: session management, connection pools and thread usage
These metrics help monitor the health and performance of your Platform HTTP endpoints running on Undertow.
